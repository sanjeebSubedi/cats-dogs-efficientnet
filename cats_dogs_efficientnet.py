...
def get_model(model_name, num_classes, input_channels, image_size):
  if vars.load_pretrained_weights:
...
def train_model(train_loader, model, optimizer, num_epochs, valid_loader=None):
  ...
  for epoch in range(1, num_epochs+1):
    ...
    print(f'Epoch:{epoch}/{num_epochs}  Acc:{round((acc_after_epoch/train_size),5)}  Loss:{loss_after_epoch/100}', end = ' ' if valid_loader is not None else '\n')
...
def validate_model(model, loader):
  ...
  with torch.no_grad():
    ...
    print(f'Val_acc:{round((num_correct/valid_size),5)}  Val_loss:{val_loss/100}')
...
if vars.load_checkpoint:
  ...
  model = get_model(vars.model_name, num_classes, input_channels=3, image_size=vars.image_size)
